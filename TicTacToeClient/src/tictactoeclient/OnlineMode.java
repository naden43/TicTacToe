...
public class OnlineMode extends BorderPane {
    ...
    static String recordGame;
    static boolean isRec;
    ...
    public OnlineMode(Stage stage, String player1, String player2, String playerx) {
        ...
        videoRec.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
    ...
    public void handleButtonClick(int index) {
        if (board[index] == '\0') {
            if (personTurn) {
                ...
                board[index] = myChar;
                recordGame += player1 + " plays " + myChar + " at " + index + "\n";
                cellsCounter++;
                ArrayList msg = new ArrayList();
                msg.add(6);
                msg.add(index);
                TicTacToeClient.playerHandler.sendRequest(gson.toJson(msg));
                GameLogic.flagOnline = true ;
                int result = GameLogic.checkWin(board, btn);
                GameLogic.flagOnline = false ;
    ...
    public static void putTheCharPlay(double index) {
        ...
        if (myChar == 'X') {
            ...
            board[(int) index] = 'O';
            recordGame += player2 + " plays O at " + ((int) index) + "\n";

        } else {
            btn[(int) index].setText("X");
            board[(int) index] = 'X';
            recordGame += player2 + " plays X at " + ((int) index) + "\n";
    ...
    private static void writeTofFile() {
        Date date = new Date();
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(player1 + "_" + player2 + date.getHours() + date.getMinutes() + ".txt", false))) {
            writer.write(recordGame);
            isRec = false;
        } catch (IOException e) {
            e.printStackTrace();
        }
